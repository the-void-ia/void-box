...
### Fixed
- **aarch64/KVM guests now boot (RFC-0003, #114).** Every arm64 Linux/KVM boot hung silently with `control_channel: deadline reached`. Root causes stacked: the loader wrote gzip-compressed `/boot/vmlinuz` bytes into guest memory unmodified (an arm64 Image has no self-decompressor, so the kernel never executed), the kernel load address ignored the Image header's `text_offset`/`image_size`, and the generated DTB lacked GIC, `/cpus`, UART, and virtio-mmio nodes — the guest kernel could not route its timer interrupt, bring up CPUs, print anywhere, or find devices. The loader now inflates gzip Images (bounded at guest-RAM size), honors the Image header with checked placement, and places the initramfs past the kernel's real footprint; the DTB describes the full platform (GICv3 with GICv2 fallback chosen by a `KVM_CREATE_DEVICE_TEST` probe, PSCI 0.2 vCPUs, an `ns16550a` UART at `0x0900_0000` so `console=ttyS0` works unchanged, and per-device virtio-mmio nodes); IRQ injection uses the arm64 `KVM_IRQ_LINE` packing (raw x86 GSI numbers select the wrong injection type on arm64); guest shutdown (`VcpuExit::SystemEvent`) stops the VM instead of spinning. Device MMIO windows and interrupt numbers derive from a shared per-arch slot table; x86_64 values and the x86_64 kernel cmdline are byte-identical, pinned by unit tests.
- **GICv3→GICv2 fallback removed in favor of probe-then-create.** KVM allows one vGIC per VM, so falling back to GICv2 after a partially-created GICv3 (address or init failure) could never succeed; the GIC version is now probed up front and a creation failure aborts the boot with a clear error.
...
- aarch64/KVM device discovery convention: devices are declared as virtio-mmio DTB nodes (no `virtio_mmio.device=` cmdline args, which cannot express GIC SPIs), and `voidbox.network=1` — previously VZ-only — is the platform-neutral guest-agent network marker.
- aarch64 vCPU ceilings derived from the guest memory map: at most 123 vCPUs (GICv3 redistributor region must stay below the UART window) and 8 on the GICv2 fallback, both rejected with clear pre-flight errors.
...
### Fixed
- **aarch64/KVM boot: vGIC initialized after vCPU creation** (`src/vmm/`). Every arm64 Linux/KVM boot failed with `KVM error: Device or resource busy (os error 16)`: `Arch::setup_vm` ran all GIC work — device creation, address attributes, `KVM_DEV_ARM_VGIC_CTRL_INIT` — before any `KVM_CREATE_VCPU`, and the arm64 KVM ABI requires the opposite order (vGIC init freezes per-vCPU redistributor state; the kernel then rejects later vCPU creation with `-EBUSY`). Since x86_64 has the inverse constraint (irqchip before vCPUs), the arch hook is now split: `setup_vm` (pre-vCPU; x86 irqchip + PIT) and `setup_vm_post_vcpus` (post-create, pre-run; all aarch64 GIC work). vCPU creation is split from run-thread start (`prepare_vcpu*` / `start_vcpu`) so the hook runs while every vCPU fd exists and none is running, on both cold-boot and snapshot-restore paths. Fixes #112. Note: arm64/KVM boot still cannot reach the guest agent until the aarch64 platform description lands (#114 — DTB GIC/cpus/UART/virtio nodes, IRQ-injection encoding).
