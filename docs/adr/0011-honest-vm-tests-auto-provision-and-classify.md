...
**Classify the `start()` result, do not probe before it.** `vm_start` attempts `backend.start()` and reads the error. A crisp capability-absence signal — the typed `Error::HypervisorUnavailable`, raised only at the `/dev/kvm` open and the required-extension check, or Apple's `Virtualization is not available on this hardware` — yields a loud skip. Every other error is a failure. `VOID_BOX_REQUIRE_VM=1` turns even a capability absence into a failure, so a CI runner asserted capable cannot launder a lost hypervisor into a skip.

**Type the capability signal; do not match errno strings.** `Error::Kvm` is `#[from]`-raised at every KVM ioctl, and on aarch64 the same errnos arise from real boot regressions (`KVM_ARM_VCPU_INIT` returns ENOENT for an unknown feature bit). Matching errno text would skip such a regression. The gate matches the typed `HypervisorUnavailable` marker instead, which is raised only at the probe sites, so a real `Error::Kvm` from any other ioctl fails.

**Consequences that follow from the two decisions above.** The GitHub macOS runner cannot virtualize, so its VZ suites skip loudly (run with `--nocapture` so the reason prints); running real VZ e2e on capable hardware is tracked in #158. `guest-agent` — the Linux-only PID-1 init — builds as an inert stub on non-Linux, so a workspace build needs no `--exclude guest-agent`. The Linux CI VM lane runs through `cargo nextest` with a bounded `vm` test group, so the suites boot a capped number of VMs at once instead of strictly serially.
