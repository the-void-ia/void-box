...
We will enforce VZ egress in-guest with a root-attached eBPF cgroup `connect4`/`connect6` program (plus `sendmsg4`/`sendmsg6` for unconnected UDP) that allow-lists egress by `(address, port)`, denies the rest with `EPERM`, and rewrites the destination to redirect allowed-but-uncooperative flows to the proxy — the in-guest equivalent of the KVM DNAT. The root guest-agent attaches the program and places the uid-1000 workload in the filtered cgroup before dropping privileges. The program itself is compiled once and shipped in the image; its per-sandbox parameters — the proxy listener `(address, port)` it rewrites to, the allow-listed destinations, DNS, and host-service ports — live in a BPF map that the guest-agent writes at sandbox start and refreshes on snapshot restore (a map update, never a per-run recompile). No kernel rebuild is required: the support an eBPF cgroup program needs — `CONFIG_CGROUP_BPF` and a mounted cgroup v2 hierarchy — is already present on both the production VZ kernel and the slim dev/test kernel. Alternatives — in-guest nftables, or replacing Apple's NAT with a host userspace stack (gvproxy-style) — were rejected for this iteration (the former is heavier and needs a kernel config flag for the slim kernel; the latter is a networking rework deserving its own RFC).
