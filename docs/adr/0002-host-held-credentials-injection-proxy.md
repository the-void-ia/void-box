...
We will keep durable credentials off the guest entirely. The host holds each provider's durable secret in a credential store. For OAuth it is also the sole owner of refresh and rotation (serialized, rate-capped, with atomic write-back of the rotated refresh token); API keys and most downstream secrets are static and simply injected, with no refresh.

The host injects the credential at network egress through a host-side **injection proxy**. The credential header lives inside the guest's TLS stream, so rewriting it requires the proxy to **terminate TLS**: it poses as the upstream, decrypts, swaps the header, and re-originates TLS to the real upstream. The client accepts the proxy as the upstream because it trusts a per-sandbox CA — a controlled, name-scoped impersonation whose mechanics are ADR-0004.

The guest never holds a real credential, only a non-secret **placeholder**: the client still needs some credential value to send, so it is handed a dummy that the proxy replaces with the host-held secret at egress. One mechanism covers API keys, OAuth, and downstream service secrets alike.

TLS termination is acceptable under this feature's single-tenant trust model — the host operator is the data owner and already sees guest plaintext (RFC-0002 §A) — and is out of scope where the operator must not read tenant data.
