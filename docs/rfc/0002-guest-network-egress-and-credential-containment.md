...
- **Guest placeholders** — non-secret dummy credential values staged into the guest in place of the real secret. The client sends a placeholder; the proxy substitutes the real secret at egress. This is what the guest holds *instead of* a credential. Substitution is a *replace*, not an *add*: the injector rewrites the scheme's credential header only where the request already carries it, and never introduces the secret into a request that presented no credential — so once the egress track routes a credentialed host's other traffic (control-plane, health checks) through the proxy, the key is never attached to an endpoint that did not ask for it.
...
**Per-sandbox token.** On top of that network-layer enforcement, each sandbox's proxy token authenticates the connection. How the client presents the token depends on which path the request takes. On the credential path the client is redirected by base URL (`ANTHROPIC_BASE_URL` and the like), so there is no `CONNECT` to carry the token; it presents the token in a request header (`x-voidbox-proxy-token`) on the redirected request. On the egress path the client reaches the proxy through an explicit `CONNECT`, so it presents the token as `Proxy-Authorization` on that `CONNECT`. Either way the proxy checks the token before any upstream connect and strips it before forwarding, so a connection is proven to come from the legitimate guest rather than a confused or hostile neighbor, and the token never leaks to the provider. Where that enforcement is host-side (KVM) the token is defense-in-depth; where it is in-guest and weaker (VZ) it is the primary cross-sandbox control. The token is guest-readable by design, so it must be a ≥128-bit CSPRNG value, compared in constant time, and never written to a log, an `EgressEvent`, or an error surface (R15).
...
**API keys (`Claude`, `codex` env-key, `Custom`).** A static secret: the proxy injects `x-api-key` (Anthropic) or `Authorization: Bearer` (OpenAI/codex; the Custom provider's configured header). No refresh, no rotation, no policy tension. codex API-key mode targets `api.openai.com/v1`; the injection is identical, but pointing codex at the proxy needs its `config.toml` base-URL knob rather than the env-only redirection M0 uses for Claude, which is why codex provisioning lands in M1b (see Rollout). The Custom provider shares Claude's header but needs base-URL-precedence and path handling before it can be redirected, so it too is deferred (see Rollout).
...
## Composing with enterprise central egress and org-managed access

void-box's egress and credential controls are a **per-sandbox enforcement point inside the untrusted-sandbox boundary**. An organization deploying it at scale already has its own control planes — a central egress path and a workload-identity model — and the design is meant to **compose beneath those planes, not compete with them**: void-box enforces what only a point inside the sandbox boundary can, and the surrounding planes do the rest.

**LLM access.** An enterprise points the agent at its own model access — an **API key**, or an internal gateway that fronts the model. This RFC's injection path already handles header-based credentials (an API key, or a bearer / configured-header token for a gateway), so a sandbox reaches that endpoint while the durable secret stays host-side. (The consumer-subscription / OAuth path in §B is the single-developer case.)

**Containment generalizes to platform identity.** Keeping the secret off the guest is not specific to provider keys: an untrusted guest equally must not hold the platform's *own* identity — a cloud instance role, say, which can call any API it is scoped for — so the same host-side injection principle applies. Today the injector covers header tokens; **native cloud-identity signing — e.g. AWS SigV4 for Amazon Bedrock — is a plausible future extension, not part of this RFC.**

**Egress layers with the central plane.** The per-sandbox enforcement point does the one thing a central egress proxy cannot: it *forces* an untrusted guest's traffic through a known point and attributes it per sandbox (a central proxy only sees traffic that already reached the cluster edge, which an adversarial guest will not send voluntarily). And because the proxy's upstream connections originate from the host, whatever central control already governs the host's egress — an egress VPC, a network firewall, PrivateLink to the model endpoint — governs void-box's traffic too, one layer up. So the two compose without either replacing the other. Tighter explicit integration — forwarding allowed egress through a corporate forward proxy, or a "delegate-egress" mode that runs no in-proxy allow-list and defers entirely upward — is a possible future direction, not in this RFC.

The throughline: void-box is the **last-hop enforcement inside the untrusted-sandbox boundary**, designed to sit beneath an organization's identity and egress planes rather than replace them. This RFC delivers that enforcement point and header-based credential injection; richer native integrations (cloud-identity signing, an upstream-proxy chain, org endpoints as first-class providers) are possible expansions on top, if demand warrants.
...
- **M0** — static-key proxy for **Claude's `x-api-key`** only. No credential store, refresh, or rotation. Ungated by V2 and by the OAuth decision; needs only V1 plus the per-sandbox name-constrained CA (R2) and the streaming proxy (R1). Establishes the shared `src/proxy/` pipeline, per-sandbox token, CA install, and lifecycle placement — all reused by egress and OAuth. The Anthropic-compatible Custom provider is deferred to M1: its `env_vars()` already emits a real `ANTHROPIC_BASE_URL` (so redirection turns on env precedence) and its base URL can carry a path the proxy's `https://host:port` redirect would drop — both need handling and a VM test first, though the injector itself already serves it. codex API-key mode (`Authorization: Bearer` to `api.openai.com`) is likewise deferred to M1b with the rest of codex: it needs `config.toml` redirection (`openai_base_url`, `CODEX_CA_CERTIFICATE`) rather than the env-only provisioning M0 uses, and its Bearer injector exists in `src/proxy/` already. M0 differs from the end-state §A / ADR-0003 posture in three recorded ways:
  - **Cross-sandbox control on KVM.** The per-sandbox network rule (§A) that denies a sandbox every gateway hop but its own listener is egress-track work M0 does not implement yet. The SLIRP gateway maps every guest's `10.0.2.2:<port>` onto the shared host loopback, so a concurrent sandbox can reach another sandbox's listener at the network layer. The per-sandbox proxy token — checked before any upstream connect, and injecting only for the sandbox that owns it — is therefore the sole cross-sandbox control on KVM in M0, not the defense-in-depth it becomes once the network rule lands.
  - **Process boundary and lifecycle.** M0 runs the guest-facing TLS/HTTP parser in-process in the host runtime and stands up a proxy handle per agent run, rather than the separate low-privilege shared process of ADR-0003 and R10. A parser compromise in M0 is therefore a host-runtime compromise; moving the parser out-of-process is a hardening step the design still requires.
  - **Auxiliary control-plane traffic is not captured.** M0 routes the credentialed host cooperatively — the base-URL redirect plus an `/etc/hosts` alias (`api.anthropic.com → gateway`) — which captures only the SDK's inference calls (`/v1/messages`, which carry the per-sandbox token). Claude Code also makes control-plane calls to its own `api.anthropic.com` endpoints — telemetry (`POST /api/event_logging/v2/batch`), settings (`GET /api/claude_code/settings`), policy limits (`GET /api/claude_code/policy_limits`), feature flags (`POST /api/eval/…`, `GET /api/claude_code_penguin_mode`), and a `HEAD /` reachability probe — that ignore the base URL and carry no token, so M0's proxy neither sees nor credentials them; on KVM they reach a gateway port with no listener and are refused. This is benign — they are best-effort, and the agent completes correctly with them failing. The egress-track transparent redirect (network-layer DNAT of 80/443 → the per-sandbox listener, destination recovered from the TLS SNI) captures any non-cooperative traffic to a credentialed host; with the per-sandbox network rule attributing these untokened flows by arrival socket, they route to the injection path and behave as in an ordinary run. Whether these calls should then be credentialed unconditionally, or only when they carry the guest placeholder, is a policy the egress track settles once that traffic actually reaches the proxy (the `path` field on `EgressEvent` audits it there).
...
- **M1b** — codex (API-key and OAuth provisioning via `config.toml` redirection, WS handling, R8), long-run rotation, and KVM+VZ parity.
...
- **eBPF object + build pipeline** — the `connect4`/`connect6`/`sendmsg` program (compiled with clang, embedded in `guest-agent`); no kernel rebuild (`CONFIG_CGROUP_BPF` + cgroup v2 are already present on both the stock VZ kernel and the slim kernel).
