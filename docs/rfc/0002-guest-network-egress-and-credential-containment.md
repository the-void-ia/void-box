...
void-box's egress and credential controls are a **per-sandbox enforcement point inside the untrusted-sandbox boundary**. An organization deploying it at scale already has its own control planes — a central egress path and a workload-identity model. The design is meant to **compose beneath those planes, not compete with them**: void-box enforces only what a point inside the sandbox boundary can, and defers everything else upward. Three points make that concrete.

**LLM access: enterprises use API keys or an org-managed endpoint — subscriptions are personal-use.** A hosted or multi-tenant deployment authenticates to the model with an **API key**, never a personal Claude/codex subscription; the consumer-OAuth path (§B) is the single-developer case and stays single-tenant (§A, ToS). Many organizations go further and front the model with an **org-managed endpoint** — for example Amazon Bedrock, Google Vertex AI, Azure OpenAI, or an internal LLM gateway — which Claude Code and codex both accept via base-URL configuration. There, authentication is the platform's (IAM/SigV4, workload identity, or a gateway token) rather than any long-lived secret void-box has to contain, so the messiest part of containment — durable consumer credentials — is often out of the picture entirely.

**Credential containment generalizes — it doesn't become moot.** Whatever the endpoint, an untrusted guest must not hold the *platform* identity: handing a uid-1000 agent the instance IAM role re-creates the same exfiltration risk, since that role can call any API it is scoped for. So the host injection proxy authenticates host-side for these endpoints too — an API key, a SigV4 signature for Bedrock, or a gateway token — while the guest holds only a placeholder. This is the same mechanism as `CredentialSpec` downstream injection (§B, ADR-0002), with the auth scheme swapped in; the injector is reused, not discarded.

**Egress composes by chaining, not competing.** The per-sandbox enforcement point (§C, ADR-0006) does the one thing a central egress proxy cannot: it *forces* an untrusted guest's traffic through a known point and attributes it per sandbox. A central proxy only sees traffic that already reached the cluster edge — which an adversarial guest will not send voluntarily. So void-box provides the bypass-resistant enforcement point, and the organization's central egress provides org-wide policy, logging, threat-intel, and DLP on top; void-box forwards allowed egress to that upstream (a forward-proxy chain or egress-VPC route) rather than duplicating its FQDN policy. Where the org's egress plane is mature, void-box can run in a **delegate-egress posture** — force traffic, isolate per sandbox, chain upstream, and run no in-proxy allow-list of its own — adding a bypass-resistant enforcement point without imposing a second policy plane.

At enterprise scale, then, void-box is the **last-hop enforcement inside the untrusted-sandbox boundary**, beneath the org's identity and egress planes. The integration knobs that make this turnkey — pluggable injector auth (including SigV4), an upstream-proxy chain, and org-managed endpoints as first-class providers — are near-term follow-ups that `CredentialSpec` and the egress profiles already shape.
