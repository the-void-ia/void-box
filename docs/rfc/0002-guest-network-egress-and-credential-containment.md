...
**Connection attribution.** Telling one run's traffic apart at a shared proxy cannot rely on an in-band marker, because transparently intercepted (DNAT'd) flows carry no header the guest could stamp. Attribution is instead structural, resting on **network-layer enforcement** that is default-deny and non-bypassable from the guest: the guest cannot open a socket the host did not allow. Concretely: each run gets a per-run proxy listener — a distinct host-side socket — and the network layer, in *every* profile including `open`, denies that run any gateway hop except its own listener (plus DNS and its enabled host-service ports). That **per-run rule** is what attributes traffic. A run therefore cannot open a connection to another run's listener, so the socket a connection arrives on is the run's identity, with no cooperation required from the guest. The per-run rule is load-bearing because the SLIRP gateway→loopback mapping preserves the port and per-VM instances share the host loopback namespace: without it, a run could dial `10.0.2.2:<another run's port>` and land on a neighbor's listener. On KVM each VM has its own SLIRP/NAT instance, so the rule is enforced host-side, per VM; on VZ, where Apple's NAT has no host filter hook, it is set in-guest by the root guest-agent before uid 1000 runs and the proxy binds a per-run address — and because in-guest enforcement is weaker than host-side, the per-run token (below) is load-bearing on VZ, not merely defense-in-depth. Listener sockets are allocated per run and released on teardown; the gateway port space bounds concurrent runs, which the implementation must size against the target VM density.
...
**Platform parity.** KVM: enforcement is host-side in the SLIRP/NAT stack (above). macOS/VZ: Apple's NAT has no host-side filter hook, so network-layer enforcement is done **in-guest** by the root guest-agent at boot (the existing `GUEST_NETWORK_DENY_LIST_PATH` mechanism, extended to express pin-to-proxy), before uid 1000 runs — so a uid-1000 agent cannot alter it. The proxy listener binds a VZ-reachable address, not loopback. Pinning and proxy reachability are validated on both platforms.
...
| E6 | **VZ in-guest enforcement** — network-layer enforcement is in-guest, not host-side. | Low–Med | Medium | Applied by the root guest-agent before uid 1000 runs; not alterable by the untrusted agent. Residual: a guest-kernel/root compromise. |
...
- `src/backend/` — VZ in-guest enforcement (`GUEST_NETWORK_DENY_LIST_PATH` extension), proxy reachability per platform.
