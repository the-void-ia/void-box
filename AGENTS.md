...
Changes that are large, complex, cross-cutting, or that need peer review before commitment go through an **RFC** (proposal + discussion) in [`docs/rfc/`](docs/rfc/README.md); the resulting load-bearing decisions are recorded as **ADRs** (atomic, immutable, superseded-not-edited) in [`docs/adr/`](docs/adr/README.md). Small, local, obvious changes skip the RFC and go straight to a PR. The full process — when to write an RFC, numbering, statuses, and templates — is documented once in [`docs/rfc/README.md`](docs/rfc/README.md).
...
Do not add AI-agent attribution anywhere — no "Generated by"/"Co-Authored-By" tool credits, no session links, no "written with <agent>" notes — in commit messages, PR titles or descriptions, code, comments, or docs. The work stands on its own; tooling used to produce it is not part of the record.

Notable changes — user-facing fixes, features, API changes, security bumps — get an entry under `[Unreleased]` in `CHANGELOG.md` in the same PR (see `CONTRIBUTING.md` and the PR-template checklist; match the existing entry style: bold lead, symptom, root cause, fix, issue references).

PR descriptions must follow `.github/PULL_REQUEST_TEMPLATE.md`, including its `Local validation` block. GitHub applies the template only in the web UI — a PR created with `gh pr create` must reproduce it in the body explicitly.

## Markdown documentation style

Conventions for Markdown prose in design docs — RFCs, ADRs, files under `docs/`, and READMEs — live in [`docs/agents/doc-style.md`](docs/agents/doc-style.md): one paragraph per line, introduce before use, mechanism before property, don't narrate, and prefer standard terms over coined jargon.

@docs/agents/doc-style.md
...
VM teardown never depends on another thread consuming a message. `MicroVm::stop()` and `snapshot()` clear `running` and kick the vCPUs first, then call `ControlChannel::shutdown()` on the VM-internal channel — killing the live multiplex channel (failing every pending dispatch slot, which unblocks an event loop wedged inside an RPC that carries no timeout of its own), aborting any in-flight connect/handshake retry loop, and refusing re-establishment — and only then join the VM threads, each join bounded by a deadline that fails with an error naming the stuck thread. The backend-level channel that carries every run's RPCs (exec, telemetry, write_file) gets the same `ControlChannel::shutdown()` in `KvmBackend::stop()` / `VzBackend::stop()`, so service-mode execs and telemetry streams fail promptly instead of waiting to observe EOF. The userspace vsock worker is stopped explicitly at the end of teardown, even when a join deadline fired (non-blocking in that case, since the wedged thread may hold the device mutex); outside teardown its lifetime is the device's lifetime, so it services the host-facing listener before driver initialization and across guest resets, with host connects and buffered guest-bound data both capped so a driver that never comes up cannot grow host memory or the fd table without bound.
